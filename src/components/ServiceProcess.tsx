...
const steps = [
    {
        icon: <Clipboard className="text-green-700 w-6 h-6 sm:w-8 sm:h-8 shrink-0 mt-1" />,
        title: "1. Предварительный аудит",
        description:
            "Оцениваем текущие процессы и выявляем ключевые риски по охране труда на вашем предприятии.",
    },
    {
        icon: <Search className="text-green-700 w-6 h-6 sm:w-8 sm:h-8 shrink-0 mt-1" />,
    ...
    {
        icon: <FileText className="text-green-700 w-6 h-6 sm:w-8 sm:h-8 shrink-0 mt-1" />,
    ...
    {
        icon: <Users className="text-green-700 w-6 h-6 sm:w-8 sm:h-8 shrink-0 mt-1" />,
    ...
    {
        icon: <CheckCircle className="text-green-700 w-6 h-6 sm:w-8 sm:h-8 shrink-0 mt-1" />,
...
const ServiceProcess: React.FC = () => {
    return (
        <section className="bg-green-700 rounded-3xl px-2 py-6 sm:p-14 my-12 sm:my-16 shadow-lg max-w-[1350px] mx-auto text-gray-900">
            ...
            <ul className="max-w-[1350px] mx-auto grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8 px-4 sm:px-0">
                {steps.map(({ icon, title, description }, idx) => (
                    <li
                        key={idx}
                        className={`flex items-start space-x-4 sm:space-x-6 group bg-green-700 rounded-xl p-4 sm:p-6 transition-colors duration-300 shadow-sm

...
const ServiceBlock: React.FC = () => {
    const items = [
        {
            icon: <FileText className="text-white w-6 h-6 sm:w-8 sm:h-8 shrink-0" />,
        ...
        {
            icon: <UserCheck className="text-white w-6 h-6 sm:w-8 sm:h-8 shrink-0" />,
        ...
        {
            icon: <CheckCircle className="text-white w-6 h-6 sm:w-8 sm:h-8 shrink-0" />,
        ...
        {
            icon: <Shield className="text-white w-6 h-6 sm:w-8 sm:h-8 shrink-0" />,
    ...
    return (
        <section className="bg-green-700/60 rounded-3xl px-2 py-6 sm:p-14 my-12 sm:my-16 shadow-lg max-w-[1350px] mx-auto text-white">
            <h2 className="text-2xl sm:text-4xl font-extrabold mb-8 sm:mb-10 text-white text-center leading-tight">
                Всё о проведении специальной оценки условий труда (СОУТ)
            </h2>

            <p className="text-base sm:text-lg leading-relaxed mb-10 max-w-[600px] sm:max-w-[900px] mx-auto text-center px-4 sm:px-0 text-white/90">
            ...
            <ul className="max-w-[1350px] mx-auto grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8 px-4 sm:px-0">
                {items.map(({ icon, title, description }, idx) => (
                    <li
                        key={idx}
                        className="flex items-start space-x-4 sm:space-x-6 group bg-[#26428b] rounded-xl p-4 sm:p-6 transition-colors duration-300 shadow-sm border border-white/20"
                    >
                        <div className="mt-1">{icon}</div>
                        <div>
                            <h3 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2 text-white leading-snug">
                                {title}
                            </h3>
